...
describe('nuki.js', function() {
  ...
  it('NukiBridge prototype methods should register/deregister nodes', function() {
    ...
    const nukiBridgeCall = RED.nodes.registerType.getCalls().find(call => call.args[0] === 'nuki-bridge');
  ...
  const config = { host: 'localhost', port: 8080, credentials: { token: 'abc' } };
  ...
  it('NukiLockControl should construct and register with bridge', function() {
    NukiModule(RED);
    const nukiLockControlCall = RED.nodes.registerType.getCalls().find(call => call.args[0] === 'nuki-lock-control');
    assert(nukiLockControlCall, 'registerType for nuki-lock-control should be called');
